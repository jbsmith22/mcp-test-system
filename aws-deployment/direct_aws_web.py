#!/usr/bin/env python3
"""
Direct AWS web interface - bypasses Lambda, connects directly to OpenSearch
"""
...
class DirectAWSInterface:
    ...
    def _run_ingestion(self, source: str, limit: int):
        ...
        try:
            self.ingestion_status["current_article"] = f"Fetching articles from {source}..."
            ...
            for i, article in enumerate(articles):
                if not self.ingestion_status["running"]:
                    break
                ...
                if self._ingest_single_article(doi, source):
...
aws_interface = DirectAWSInterface()

@app.route('/')
def index():
    """Main page"""
    return render_template('direct_aws.html')
...
def search():
    ...
    result = aws_interface.search_articles(query, limit, search_type)
    return jsonify(result)

@app.route('/stats')
def stats():
    """Database statistics"""
    result = aws_interface.get_database_stats()
    return jsonify(result)

@app.route('/sources')
def sources():
    """Available ingestion sources"""
    sources = aws_interface.get_available_sources()
...
def ingest():
    ...
    result = aws_interface.start_ingestion(source, limit)
    return jsonify(result)

@app.route('/ingestion-status')
def ingestion_status():
    """Get ingestion status"""
    status = aws_interface.get_ingestion_status()
    return jsonify(status)

@app.route('/stop-ingestion', methods=['POST'])
def stop_ingestion():
    """Stop ingestion"""
    result = aws_interface.stop_ingestion()
    return jsonify(result)

if __name__ == '__main__':
    print("🚀 Starting Direct AWS NEJM Research Assistant")
    print("=" * 50)
    print("✅ AWS OpenSearch: Direct connection")
    print("✅ Bedrock embeddings: Ready")
    print("✅ NEJM API: Configured")
    print("✅ Ingestion: Available")
    print("✅ No Lambda dependency")
